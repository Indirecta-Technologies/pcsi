...
local IN, OUT, SAME = "in", "out", "same" -- token statuses
...
function diff(old, new, separator)
	...
	new = string.split(new, separator)
	old = string.split(old, separator)
	...
	local rev_diff = {
		...
		ins = function(self, token)
			self:put(token, IN)
		end,
		del = function(self, token)
			self:put(token, OUT)
		end,
		same = function(self, token)
			if token then
				self:put(token, SAME)
			end
		end,
	...
	-----------------------------------------------------------------------------
	-- Formats an inline diff as RICHTEXT, with <ins> and <del> tags.
	--
	-- @param tokens         a table of {token, status} pairs.
	-- @return               a richtext string.
	-----------------------------------------------------------------------------
	local function format_as_richtext(tokens)
		local diff_buffer = ""
		local token, status
		for i, token_record in ipairs(tokens) do
			token = token_record[1]:gsub("&", "&amp;"):gsub("<", "&lt;"):gsub(">", "&gt;"):gsub('"', "&quot;"):gsub("'", "&apos;") --game:GetService("HttpService"):UrlEncode(token_record[1])
			status = token_record[2]
			if status == "in" then
				diff_buffer = diff_buffer .. '<font color="#00aa00">' .. token .. " </font>"
			elseif status == "out" then
				diff_buffer = diff_buffer .. '<font color="#aa0000">' .. token .. " </font>"
			else
	...
	diff.richtext = format_as_richtext(diff)
	return diff
end
