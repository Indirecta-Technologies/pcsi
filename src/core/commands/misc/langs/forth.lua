...
local cmd = {
	...
	fn = function(plr, pCsi, essentials, args)
		...
		-- Clear the screen
		WORDS.PAGE[1] = function()
			pCsi.io.write("\x1b[31m")
		end
		...
		local function interpret(input)
			local splitInput = pCsi.libs.text.internal.tokenize(input)
		...
		local repl = true
		if #args > 0 then
			if pCsi.xfs.exists(args[1]) then
				repl = false
				interpret( pCsi.xfs.read(args[1]))
			end
		end
		while repl do
			if not IDW then
