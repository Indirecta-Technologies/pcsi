local cmd = {
	...
	fn = function(plr, pCsi, essentials, args)
		...
		function commands.wait()
			pCsi.io.write("Time passes...")
		end

		function commands.look(_sTarget)
			local room = getRoom(x, y, z)
			if room.dark then
				pCsi.io.write("It is pitch dark.")
				return
			end
			...
			if _sTarget == nil then
				-- Look at the world
				if y == 0 then
					pCsi.io.write("You are standing " .. tBiomes[room.nBiome] .. ". ")
					pCsi.io.write(tDayCycle[getTimeOfDay()])
				else
					pCsi.io.write("You are underground. ")
					if next(room.exits) ~= nil then
						pCsi.io.write("You can travel " .. itemize(room.exits) .. ".")
					else
						pCsi.io.write()
					end
				end
				if next(room.items) ~= nil then
					pCsi.io.write("There is " .. itemize(room.items) .. " here.")
				end
				if room.trees then
					pCsi.io.write("There are trees here.")
				end
			else
				-- Look at stuff
				if room.trees and (_sTarget == "tree" or _sTarget == "trees") then
					pCsi.io.write("The trees look easy to break.")
				elseif _sTarget == "self" or _sTarget == "myself" then
					pCsi.io.write("Very handsome.")
				else
					local tItem = nil
					...
					if tItem then
						pCsi.io.write(tItem.desc or ("You see nothing special about " .. sItem .. "."))
					else
						pCsi.io.write("You don't see any " .. _sTarget .. " here.")
					end
				end
			end
		end
		...
		function commands.go(_sDir)
			...
			if room.exits[_sDir] == nil then
				pCsi.io.write("You can't go that way.")
				return
			end
			...
			elseif _sDir == "down" then
				...
				pCsi.io.write("I don't understand that direction.")
				return
			end
		...
		function commands.dig(_sDir, _sTool)
			...
			if _sTool ~= nil then
				...
				if not sTool then
					pCsi.io.write("You're not carrying a " .. _sTool .. ".")
					return
				end
			...
			if bActuallyDigging then
				if sTool == nil or tTool.toolType ~= "pick" then
					pCsi.io.write("You need to use a pickaxe to dig through stone.")
					return
				end
			end
			...
			elseif _sDir == "up" then
				if y == 0 then
					pCsi.io.write("You can't dig that way.")
					return
				end
			...
			--
			if bActuallyDigging then
				if _sDir == "down" and y == -1 or _sDir == "up" and y == 0 then
					inventory["some dirt"] = items["some dirt"]
					inventory["some stone"] = items["some stone"]
					pCsi.io.write("You dig " .. _sDir .. " using " .. sTool .. " and collect some dirt and stone.")
				else
					inventory["some stone"] = items["some stone"]
					pCsi.io.write("You dig " .. _sDir .. " using " .. sTool .. " and collect some stone.")
				end
			end
		...
		function commands.inventory()
			pCsi.io.write("You are carrying " .. itemize(inventory) .. ".")
		end

		function commands.drop(_sItem)
			if _sItem == nil then
				pCsi.io.write("Drop what?")
				return
			end
			...
			if sItem then
				...
				if tItem.droppable == false then
					pCsi.io.write("You can't drop that.")
				else
					room.items[sItem] = tItem
					inventory[sItem] = nil
					pCsi.io.write("Dropped.")
				end
			else
				pCsi.io.write("You don't have a " .. _sItem .. ".")
			end
		end

		function commands.place(_sItem)
			if _sItem == nil then
				pCsi.io.write("Place what?")
				return
			end
			...
			if _sItem == "torch" or _sItem == "a torch" then
				...
				if inventory["some torches"] or inventory["a torch"] then
					...
					if room.dark then
						pCsi.io.write("The cave lights up under the torchflame.")
						room.dark = false
					elseif y == 0 and not isSunny() then
						pCsi.io.write("The night gets a little brighter.")
					else
						pCsi.io.write("Placed.")
					end
				else
					pCsi.io.write("You don't have torches.")
				end
				return
			end
		...
		function commands.take(_sItem)
			...
			if sItem then
				...
				if tItem.heavy == true then
					pCsi.io.write("You can't carry " .. sItem .. ".")
				elseif tItem.ore == true then
					pCsi.io.write("You need to mine this ore.")
				else
					...
					if sItem == "a torch" and y < 0 then
						room.dark = true
						pCsi.io.write("The cave plunges into darkness.")
					else
						pCsi.io.write("Taken.")
					end
				end
			else
				pCsi.io.write("You don't see a " .. _sItem .. " here.")
			end
		end

		function commands.mine(_sItem, _sTool)
			if _sItem == nil then
				pCsi.io.write("Mine what?")
				return
			end
			if _sTool == nil then
				pCsi.io.write("Mine " .. _sItem .. " with what?")
				return
			end
		...
		function commands.cbreak(_sItem, _sTool)
			...
			if _sItem == "tree" or _sItem == "trees" or _sItem == "a tree" then
				pCsi.io.write("The tree breaks into blocks of wood, which you pick up.")
			...
			if sItem then
				...
				if tItem.ore == true then
					-- Breaking ore
					if not sTool then
						pCsi.io.write("You need a tool to break this ore.")
						return
					end
					local tTool = inventory[sTool]
					if tTool.tool then
						if tTool.toolLevel < tItem.toolLevel then
							pCsi.io.write(sTool .. " is not strong enough to break this ore.")
						elseif tTool.toolType ~= tItem.toolType then
							pCsi.io.write("You need a different kind of tool to break this ore.")
						else
							pCsi.io.write("The ore breaks, dropping " .. sItem .. ", which you pick up.")
						...
						pCsi.io.write("You can't break " .. sItem .. " with " .. sTool .. ".")
					end
				elseif tItem.creature == true then
					...
					if math.random() <= tChances[toolLevel + 1] then
						room.items[sItem] = nil
						pCsi.io.write("The " .. tItem.aliases[1] .. " dies.")

						if tItem.drops then
							for n, sDrop in pairs(tItem.drops) do
								if not room.items[sDrop] then
									pCsi.io.write("The " .. tItem.aliases[1] .. " dropped " .. sDrop .. ".")
						...
						pCsi.io.write("The " .. tItem.aliases[1] .. " is injured by your blow.")
					end
					...
					pCsi.io.write("You can't break " .. sItem .. ".")
				end
			else
				pCsi.io.write("You don't see a " .. _sItem .. " here.")
			end
			...
			if tRecipe then
				for n, sReq in ipairs(tRecipe) do
					if inventory[sReq] == nil then
						pCsi.io.write("You don't have the items you need to craft " .. sItem .. ".")
						return
					end
				end
				...
				pCsi.io.write("Crafted.")
			else
				pCsi.io.write("You don't know how to make " .. (sItem or _sItem) .. ".")
			end
		end

		function commands.build(_sThing, _sMaterial)
			if _sThing == nil then
				pCsi.io.write("Build what?")
				return
			end
			...
			if _sMaterial == nil then
				...
				if sMaterial == nil then
					pCsi.io.write("You don't have any building materials.")
					return
				end
			else
				sMaterial = findItem(inventory, _sMaterial)
				if not sMaterial then
					pCsi.io.write("You don't have any " .. _sMaterial)
					return
				end

				if inventory[sMaterial].material ~= true then
					pCsi.io.write(sMaterial .. " is not a good building material.")
					return
				end
			end
			...
			room.items[_sThing] = {
				heavy = true,
				aliases = { alias },
				desc = "As you look at your creation (made from "
					.. sMaterial
					.. "), you feel a swelling sense of pride.",
			}

			pCsi.io.write("Your construction is complete.")
		end
		...
		function commands.eat(_sItem)
			if _sItem == nil then
				pCsi.io.write("Eat what?")
				return
			end

			local sItem = findItem(inventory, _sItem)
			if not sItem then
				pCsi.io.write("You don't have any " .. _sItem .. ".")
				return
			end

			local tItem = inventory[sItem]
			if tItem.food then
				pCsi.io.write("That was delicious!")
				inventory[sItem] = nil
				...
				pCsi.io.write("You can't eat " .. sItem .. ".")
			end
		end
		...
		function commands.badinput()
			...
			pCsi.io.write(tResponses[math.random(1, #tResponses)])
		end
				...
				for sy = -1, 1 do
					for sz = -2, 2 do
						...
						if h >= -3 and h <= 0 then
							...
							if
								room.nMonsters < 2
								and ((h == 0 and not isSunny() and not room.items["a torch"]) or room.dark)
								and math.random(1, 6) == 1
							then
								...
								if room.items[sMonster] == nil then
									...
									if sx == 0 and sy == 0 and sz == 0 and not room.dark then
										pCsi.io.write("From the shadows, " .. sMonster .. " appears.")
							...
							if h == 0 and isSunny() then
								for n, sMonster in ipairs(tMonsters) do
									if room.items[sMonster] and items[sMonster].nocturnal then
										...
										if sx == 0 and sy == 0 and sz == 0 and not room.dark then
											pCsi.io.write(
												"With the sun high in the sky, the "
													.. items[sMonster].aliases[1]
													.. " bursts into flame and dies."
											)
										end
										room.nMonsters = room.nMonsters - 1
									end
			...
			if nTimeInRoom >= 2 and not bNewMonstersThisRoom then
				for n, sMonster in ipairs(tMonsters) do
					if room.items[sMonster] then
						if math.random(1, 4) == 1 and not (y == 0 and isSunny() and (sMonster == "a spider")) then
							if sMonster == "a creeper" then
								if room.dark then
									pCsi.io.write("A creeper explodes.")
								else
									pCsi.io.write("The creeper explodes.")
								end
								room.items[sMonster] = nil
								room.nMonsters = room.nMonsters - 1
							else
								if room.dark then
									pCsi.io.write("A " .. items[sMonster].aliases[1] .. " attacks you.")
								else
									pCsi.io.write("The " .. items[sMonster].aliases[1] .. " attacks you.")
								end
							end
		...
		while bRunning do
			...
			local sRawLine = pCsi.io.read()
			pCsi.io.write("> " .. sRawLine)
